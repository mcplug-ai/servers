...
async function processBatchOperation(operation: QueuedBatchOperation): Promise<void> {
  try {
    ...
    switch (name) {
      ...
      case "firecrawl_batch_scrape": {
        ...
        try {
          ...
          const errorMessage =
            error instanceof Error ? error.message : `Batch operation failed: ${JSON.stringify(error)}`;
          return {
            content: [{ type: "text", text: trimResponseText(errorMessage) }],
            isError: true
          };
...
${result.markdown ? `\nContent:\n${result.markdown}` : ""}`
      ...
      case "firecrawl_extract": {
        ...
        try {
          ...
          if (FIRECRAWL_API_URL && errorMessage.toLowerCase().includes("not supported")) {
